...
const stats = function () {
    const {totalGames, bestStreak, currentStreak, successRate, gamesFailed, winDistributions} = getStats("gameStats");
...
<h4 class="cursor-pointer text-lg leading-6 font-medium text-gray-900 dark:text-gray-100" id="showHide">Show Guess Distribution</h4><div class="columns-1 justify-left m-2 text-sm dark:text-white">`
    ...
    let sum = winDistributions.reduce(function (previousValue, currentValue) {
    ...
    for(let i=1; i<=8; i++){
        blocks += `<div class="flex justify-left m-1">
                        ...
                        <div class="rounded-full w-full ml-2">
                            <div class="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 rounded-l-full" style="width: ${Math.ceil(winDistributions[i] / sum * 100) }%;">
                                ${winDistributions[i]}
                            </div>
                        </div>
                    </div>`
